...
- **过期丢弃**：主模型回复生成期间，如果新的群聊消息也值得回复，旧回复会在发送前被丢弃
...
```
判断小模型 (用户配置) → 快速决策 → AstrBot完整主流程调用大型回复模型 → 发送最新有效回复
```

同一群聊只保留最新的心流主动回复版本。新的消息如果再次被小模型判断为值得回复，会让旧回复在发送前失效，避免讨论结束后旧回复排队刷屏，同时保留 AstrBot 默认的人格、记忆、工具和结果装饰流程。
...
- `drop_stale_replies`：新心流回复触发后丢弃旧回复 (默认true)
